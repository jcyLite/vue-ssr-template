<template>
  <div class="pot-upload-file" @click="clickHandler">
    <slot
      :img-style="fileStyle"
      :progress="fileProgress"
    >
      <div class="pot-upload-file-def" :style="fileStyle">
        <i class="potic-wrong" @click.stop="removeFile"></i>
        <div class="pot-upload-file-state" :class="fileStatusCls">
          <i class="pot-upload-file-status" :class="statusCls"></i>
          <span class="pot-upload-file-progress">{{fileProgress}}</span>
        </div>
      </div>
    </slot>
  </div>
...
<script type="text/ecmascript-6">
  ...
  const COMPONENT_NAME = 'pot-upload-file'
...
<style lang="stylus" rel="stylesheet/stylus">
  ...
  .pot-upload-file_stat
    opacity: 1

  .pot-upload-file-status
    position: relative
    z-index: 1
    font-size: $fontsize-large-xxxx
    display: none

  .pot-upload-file-status.potic-right
    display: block
    color: $upload-file-success-color

  .pot-upload-file-status.potic-warn
    display: block
    color: $upload-file-error-color

  .pot-upload-file-status.potic-right, .pot-upload-file-status.potic-warn
    &::after
      content: ""
      z-index: -1
      position: absolute
