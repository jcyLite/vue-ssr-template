<template>
  <div class="pot-validator" :class="containerClass">
    <div class="pot-validator-content">
      <slot></slot>
    </div>
    <div class="pot-validator-msg" @click="msgClickHandler">
      <slot
        name="message"
        :message="msg"
        :dirty="dirty"
        ...
        :result="result"
      >
        <span class="pot-validator-msg-def">{{ dirtyOrValidated ? msg : '' }}</span>
      </slot>
    </div>
  </div>
</template>
...
<script type="text/ecmascript-6">
  ...
  const COMPONENT_NAME = 'pot-validator'
  ...
  export default {
    ...
    computed: {
      ...
      containerClass() {
        ...
        return {
          'pot-validator_invalid': this.invalid,
          'pot-validator_valid': this.valid,
          'pot-validator_validating': this.validating
        }
      }
    },
    watch: {
    ...
    methods: {
      ...
      findMessage (key, config, type, val) {
        const messages = this.$potMessages
        const lang = this.$potLang
