...
const locale = {
  ...
  install (Vue) {
    ...
    proto = Vue.prototype
    Vue.util.defineReactive(proto, '$potLang', DEFAULT_LANG)
    proto['$potMessages'] = { [DEFAULT_LANG]: defaultMessages }
    locale.installed = true
  },
  use (lang, messages) {
    proto['$potLang'] = lang
    const potMessages = proto['$potMessages']
    // if messages have never been stored in vue.prototye
    if (!(lang in potMessages)) {
      potMessages[[lang]] = messages
    }
  },
  helpers: {
